...
- (void)encodeBuffer:(char*)buf timeStamp:(uint64_t)timeStamp{
    ...
    // audioData = [PCMFileManager denoiseData:audioData config:self.configuration];
    ...
    int samplesCount = seconds * 1000 / 10 * ((uint32_t)self.configuration.audioSampleRate / 100.0);
    if (samplesCount > 0) {
        audioData = [PCMFileManager nsProcess:audioData sampleRate:(uint32_t)self.configuration.audioSampleRate channels:(uint32_t)self.configuration.numberOfChannels samplesCount:samplesCount level:2];
    }
    */
